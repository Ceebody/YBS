...
const PHOTO_MAPPING = {
    ...
    "Adu Darko, Samuel": { url: "assets/Adu Darko, Samuel.png", position: "center 40%" },
    "Agbleke, Elinam": { url: "assets/Agbleke, Elinam.png" },
    "Amponsah-Sam, Johnson Chris": { url: "assets/Amponsah-Sam, Johnson Chris.png" },
    "Asante, Diana": { url: "assets/Asante, Diana.png", position: "center 45%" },
    ...
    "Nartey, Shallom": { url: "assets/Nartey, Shallom.png", position: "center 30%" },
...
function getCandidatePhotoStyle(candidate) {
    ...
    if (mapping) {
        if (mapping.zoom) {
            styles += `background-size: ${mapping.zoom}; `;
        }
        if (mapping.zoomOut) {
            styles += "background-size: contain; ";
        }
        if (mapping.position) {
            styles += `background-position: ${mapping.position} !important; `;
        }
    }
    
    // Default fallback if a specific property wasn't overridden
    if (!styles.includes('background-repeat')) styles += "background-repeat: no-repeat; ";
